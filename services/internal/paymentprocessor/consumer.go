...
func (consumer *Consumer) HandleRecord(ctx context.Context, record *kgo.Record) (err error) {
	defer func() {
		...
		telemetry.ServiceMetrics("payment-processor").RecordKafka(record.Topic, outcome)
	}()
	...
	for {
		...
		fetches.EachRecord(func(record *kgo.Record) {
			...
			if err := consumer.handler.HandleRecord(ctx, record); err != nil && ctx.Err() == nil {
				telemetry.ServiceMetrics("payment-processor").RecordKafka(record.Topic, "retried")
