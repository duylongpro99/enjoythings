...
- Transition to `PAYMENT_PROCESSING` after both outbox records are durable.
...
Extend saga persistence with nullable fraud metadata and deferred payment-result JSON. Do not overload `last_error` with fraud details.
